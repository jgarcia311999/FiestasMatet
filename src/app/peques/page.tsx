...
export default async function Peques() {
  ...
  return (
    <main className="min-h-screen bg-[#a3b18a] text-[#0C2335]">
      <div className="mx-auto max-w-sm px-1 pt-10 pb-24">
        <h1 className="font-serif text-[36px] leading-[1.05] tracking-tight text-[#0C2335]">
          Disfruta de todos los <strong>eventos familiares</strong> de <strong>MATET</strong>
        </h1>
        ...
        {secciones.length === 0 ? (
          ...
          secciones.map((sec) => (
            <div key={sec.key}>
              <div className="text-lg uppercase tracking-[0.18em] py-2 cursor-pointer text-[#0C2335]">
                <span className="border-b border-transparent">{sec.label}</span>
              </div>
              <div className="p-2 text-base">
                {getEventosPorFecha(allEvents, sec.key).length === 0 ? (
                  <div className="italic text-[#0C2335]">Sin eventos para este día</div>
                ) : (
        ...
        <div className="mt-8">
          <p className="font-serif text-[28px] leading-tight text-[#0C2335]">Matet</p>
          <p className="font-serif text-[28px] leading-tight text-[#0C2335]">es su gente</p>
          <p className="mt-2 text-[12px] text-[#0C2335]">@comision2026</p>
        </div>
      </div>
    </main>
  );
